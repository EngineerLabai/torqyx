...
export function getToolHeroImage(toolId?: string | null): string {
  ...
  return mapping[toolId] ?? HERO_ASSETS.toolDetail ?? HERO_PLACEHOLDER;
}
...
export function listPublicImagePaths(): string[] {
  return Array.from(new Set([...listHeroImagePaths(), ...listBlueprintImagePaths(), "/images/logo.png"])).sort();
}
